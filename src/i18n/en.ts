export default {
  common: {
    appName: "Pilates Campus",
    signOut: "Sign Out",
    navigation: "Navigation",
    cancel: "Cancel",
    create: "Create",
    update: "Update",
    save: "Save",
    delete: "Delete",
    add: "Add",
    edit: "Edit",
    or: "or",
    noData: "No data",
    loading: "Loading…",
    error: "Error",
    search: "Search…",
    showing: "{{from}}–{{to}} of {{total}}",
    all: "All",
  },
  roles: {
    admin: "Administrator",
    trainer: "Trainer",
    trainee: "Trainee",
  },
  login: {
    title: "Pilates Campus",
    subtitle: "Manage your training, schedule sessions, track progress",
    login: "Log In",
    signup: "Sign Up",
    email: "Email",
    password: "Password",
    fullName: "Full Name",
    signingIn: "Signing in…",
    signIn: "Sign In",
    creatingAccount: "Creating account…",
    createAccount: "Create Account",
    continueGoogle: "Continue with Google",
    loginFailed: "Login failed",
    signupFailed: "Signup failed",
    checkEmail: "Check your email",
    confirmationSent: "We sent you a confirmation link.",
    googleFailed: "Google login failed",
    rememberMe: "Remember me",
    forgotPassword: "Forgot password?",
    enterEmailFirst: "Please enter your email first",
    resetFailed: "Password reset failed",
    resetEmailSent: "Reset email sent",
    resetEmailSentDesc: "Check your inbox for a password reset link.",
  },
  resetPassword: {
    title: "Reset Password",
    description: "Enter your new password below.",
    newPassword: "New Password",
    confirmPassword: "Confirm Password",
    update: "Update Password",
    updating: "Updating…",
    success: "Password updated successfully",
    failed: "Failed to update password",
    mismatch: "Passwords do not match",
    tooShort: "Password must be at least 6 characters",
    invalidLink: "This reset link is invalid or has expired.",
    backToLogin: "Back to Login",
  },
  nav: {
    dashboard: "Dashboard",
    trainers: "Trainers",
    trainees: "Trainees",
    packages: "Packages",
    schedule: "Schedule",
    pricing: "Pricing",
    reports: "Reports",
    mySchedule: "My Schedule",
    sessionHistory: "Session History",
    notifications: "Notifications",
    myPackage: "My Package",
    bookSessions: "Book Sessions",
    myBookings: "My Bookings",
    myProfile: "My Profile",
    cancellation: "Cancellation Policy",
    compensation: "Compensation",
    bookForTrainee: "Book for Trainee",
    manageBookings: "Manage Bookings",
    promotions: "Promotions",
    emailSettings: "Email Settings",
    auditLog: "Audit Log",
  },
  bookOnBehalf: {
    title: "Book on Behalf of Trainee",
    selectTrainee: "Select Trainee",
    chooseTrainee: "Choose a trainee…",
    selectTraineeFirst: "Please select a trainee first",
    noPackage: "no package",
    bookedSuccess: "Session booked for {{name}}",
    notificationTitle: "Session Booked for You",
    notificationMessage: "You have been booked for {{class}} on {{date}} at {{time}}.",
  },
  manageBookings: {
    title: "Manage Bookings",
    trainee: "Trainee",
    class: "Class",
    dateTime: "Date & Time",
    type: "Type",
    status: "Status",
    noBookings: "No confirmed bookings",
    cancelledFor: "Booking cancelled for {{name}}",
    noRefundApplied: "No refund applied (too close to session).",
  },
  admin: {
    dashboard: {
      title: "Admin Dashboard",
      totalTrainees: "Total Trainees",
      trainers: "Trainers",
      upcomingSessions: "Upcoming Sessions",
      activePackages: "Active Packages",
    },
    trainers: {
      title: "Trainers",
      addTrainer: "Add Trainer",
      userEmail: "User Email",
      specialty: "Specialty",
      name: "Name",
      email: "Email",
      noTrainers: "No trainers yet",
      userNotFound: "User not found",
      userNotFoundDesc: "No account with that email exists.",
      trainerAdded: "Trainer added",
      trainerRemoved: "Trainer removed",
      adding: "Adding…",
      level: "Level",
      levelUpdated: "Trainer level updated",
    },
    compensation: {
      title: "Trainer Compensation",
      description: "Configure compensation rates per trainer level. Each level can have a fixed amount or percentage rate applied per completed session.",
      level: "Level",
      rateType: "Rate Type",
      rateValue: "Rate Value",
      editRate: "Edit Rate",
      noRates: "No compensation rates configured",
      rateUpdated: "Compensation rate updated",
      fixedLabel: "Amount per session",
      percentageLabel: "Percentage (%)",
      levels: {
        trainee_trainer: "Trainee Trainer",
        junior: "Junior",
        senior: "Senior",
        master: "Master",
      },
      types: {
        fixed: "Fixed",
        percentage: "Percentage",
      },
    },
    trainees: {
      title: "Trainees",
      name: "Name",
      email: "Email",
      activePackage: "Active Package",
      creditsLeft: "Credits Left",
      none: "None",
      noTrainees: "No trainees yet",
      assignPackage: "Assign Package",
      selectPackage: "Select a package",
      packageAssigned: "Package assigned successfully",
      assignFailed: "Failed to assign package",
      assigning: "Assigning…",
      actions: "Actions",
      editTrainee: "Edit Trainee",
      credits: "Credits",
      updateSuccess: "Trainee updated successfully",
      updateFailed: "Failed to update trainee",
      saving: "Saving…",
      bonusAwarded: "{{credits}} bonus credits awarded ({{type}})",
      inviteTrainees: "Invite Trainees",
      inviteTitle: "Invite New Trainees",
      inviteDesc: "Enter email addresses, one per line or comma-separated.",
      inviteEmails: "Email addresses",
      invitePlaceholder: "email1@example.com\nemail2@example.com",
      sending: "Sending…",
      sendInvites: "Send Invites",
      inviteSuccess: "{{count}} invitation(s) sent",
      inviteAlreadyExists: "{{count}} already have accounts: {{emails}}",
      inviteFailed: "{{count}} failed to send",
      inviteNoValid: "No valid email addresses found",
      changeRole: "Change Role",
      changeRoleDesc: "Change this user's role. This will take effect immediately.",
      selectRole: "Select new role",
      roleChanged: "Role changed successfully",
    },
    packages: {
      title: "Packages",
      newPackage: "New Package",
      editPackage: "Edit Package",
      createPackage: "Create Package",
      name: "Name",
      description: "Description",
      credits: "Credits",
      price: "Price ($)",
      expiryDays: "Expiry (days)",
      active: "Active",
      expiry: "Expiry",
      noPackages: "No packages yet",
      packageCreated: "Package created",
      packageUpdated: "Package updated",
    },
    schedule: {
      title: "Class Schedule",
      newClass: "New Class",
      createClass: "Create Class",
      classTitle: "Title",
      trainer: "Trainer",
      selectTrainer: "Select trainer",
      startDate: "Start Date",
      endDate: "End Date",
      startTime: "Start Time",
      endTime: "End Time",
      capacity: "Capacity",
      type: "Type",
      group: "Group",
      private: "Private",
      recurrenceDays: "Recurrence Days",
      noClasses: "No classes scheduled",
      classCreated: "Class created with {{count}} slots",
      classDeleted: "Class and all slots deleted",
      slotsCount: "{{count}} slots",
      mon: "Mon",
      tue: "Tue",
      wed: "Wed",
      thu: "Thu",
      fri: "Fri",
      sat: "Sat",
      sun: "Sun",
    },
    reports: {
      title: "Reports",
      attendance: "Attendance",
      attendanceDesc: "Attendance reports and charts will appear here once sessions are booked.",
      packageUtilization: "Package Utilization",
      packageUtilizationDesc: "Package usage and revenue data will be displayed here.",
    },
    pricing: {
      title: "Time-Based Pricing",
      description: "Configure credit costs for different times of day. Trainees will be charged the corresponding credits when booking a session. Default cost is 1 credit.",
      addPeriod: "Add Time Period",
      editPeriod: "Edit Time Period",
      label: "Label",
      tier: "Tier",
      startTime: "Start Time",
      endTime: "End Time",
      timeRange: "Time Range",
      creditCost: "Credit Cost",
      credit: "credit",
      credits: "credits",
      peak: "Peak",
      shoulder: "Shoulder",
      off_peak: "Off-Peak",
      periodCreated: "Time period created",
      periodUpdated: "Time period updated",
      periodDeleted: "Time period deleted",
      noPeriods: "No time periods configured",
    },
    cancellation: {
      title: "Cancellation Policy",
      description: "Configure tiered cancellation rules. Tiers are matched from highest hours_before downward. The first tier where the trainee's advance notice meets or exceeds hours_before applies.",
      addTier: "Add Tier",
      editTier: "Edit Tier",
      hoursBefore: "Hours Before Session",
      refundPercentage: "Refund %",
      tierCreated: "Cancellation tier created",
      tierUpdated: "Cancellation tier updated",
      tierDeleted: "Cancellation tier deleted",
      noTiers: "No cancellation tiers configured",
      lessThanMin: "Less than minimum",
    },
    emailSettings: {
      title: "Email Settings",
      senderInfo: "Sender Information",
      senderInfoDesc: "Configure the display name and reply-to address for outgoing emails.",
      senderName: "Sender Name",
      replyTo: "Reply-To Email",
      templates: "Email Templates",
      templatesDesc: "Customize the subject and body text for each email type.",
      subject: "Subject",
      body: "Body",
      saveSuccess: "Email settings saved",
      saveFailed: "Failed to save email settings",
      template: {
        invitation: "Invitation",
        booking_reminder: "Booking Reminder",
        low_credits: "Low Credits Warning",
        package_expiry: "Package Expiry Notice",
        promotion: "Promotion",
      },
    },
    promotions: {
      title: "Promotions",
      description: "Manage promotional programs: first-time bonuses, campaigns, vouchers, referral rewards, and returning customer incentives.",
      addPromotion: "Add Promotion",
      editPromotion: "Edit Promotion",
      promoName: "Name",
      promoType: "Type",
      credits: "Credits",
      dateRange: "Date Range",
      status: "Status",
      startDate: "Start Date",
      endDate: "End Date (optional)",
      noPromotions: "No promotions yet",
      created: "Promotion created",
      updated: "Promotion updated",
      deleted: "Promotion deleted",
      types: {
        first_time: "First-Time",
        campaign: "Campaign",
        voucher: "Voucher",
        referral: "Referral",
        returning: "Returning",
      },
      voucherCodes: "Voucher Codes",
      generateVouchers: "Generate Codes",
      code: "Code",
      expiresAt: "Expires",
      redeemed: "Redeemed",
      expired: "Expired",
      available: "Available",
      noVouchers: "No voucher codes yet",
      vouchersGenerated: "{{count}} voucher codes generated",
      codeCopied: "Code copied to clipboard",
      sendCampaign: "Send Campaign",
      campaignSent: "Campaign sent to all trainees",
    },
    auditLog: {
      title: "Audit Log",
      description: "Track all data changes across the system — who created, updated, or deleted records.",
      filterTable: "Filter by table",
      filterAction: "Filter by action",
      searchUser: "Search by user…",
      dateTime: "Date & Time",
      user: "User",
      action: "Action",
      table: "Table",
      recordId: "Record ID",
      noLogs: "No audit logs found",
      details: "Change Details",
      oldData: "Previous Data",
      newData: "New Data",
      system: "System",
    },
  },
  trainer: {
    schedule: {
      title: "My Schedule",
      class: "Class",
      dateTime: "Date & Time",
      type: "Type",
      trainees: "Trainees",
      noBookings: "No bookings",
      noSessions: "No upcoming sessions",
    },
    history: {
      title: "Session History",
      empty: "Past sessions will appear here once they are completed.",
    },
    profile: {
      title: "My Profile",
      personalInfo: "Personal Information",
      fullName: "Full Name",
      phone: "Phone",
      specialty: "Specialty",
      bio: "Bio",
      updateSuccess: "Profile updated successfully",
      updateFailed: "Failed to update profile",
      changeEmail: "Change Email",
      currentEmail: "Current Email",
      newEmail: "New Email",
      newEmailPlaceholder: "Enter new email address",
      emailChangeNote: "A confirmation link will be sent to both your current and new email address. Your email will update once you confirm.",
      emailChangeRequested: "Confirmation email sent! Check both your current and new email inbox.",
      invalidEmail: "Please enter a valid email address",
      sameEmail: "New email is the same as your current email",
    },
  },
  trainee: {
    package: {
      title: "My Package",
      noPackage: "You don't have an active package. Contact your admin to get one assigned.",
      active: "Active",
      inactive: "Inactive",
      creditsRemaining: "Credits remaining",
      sessionsUsed: "Sessions used",
      expires: "Expires",
    },
    booking: {
      title: "Book Sessions",
      bookRecurring: "Book recurring (weekly)",
      creditsRemaining: "Credits remaining",
      classesOn: "Classes on {{date}}",
      spots: "{{booked}}/{{total}} spots",
      full: "Full",
      bookWeekly: "Book Weekly",
      book: "Book",
      creditsCost: "{{cost}} credit(s)",
      noActivePackage: "No active package",
      needPackage: "You need an active package to book.",
      classFull: "Class full",
      classFullDesc: "This class has reached its capacity.",
      noCredits: "No credits left",
      noMatchingSlots: "No matching slots",
      noMatchingSlotsDesc: "No future recurring slots found.",
      recurringCreated: "Recurring bookings created",
      sessionsBooked: "{{count}} sessions booked.",
      sessionBooked: "Session booked!",
      alreadyBooked: "Already booked",
      alreadyBookedDesc: "This trainee is already booked for this class.",
      noClasses: "No classes on this day",
    },
    bookings: {
      title: "My Bookings",
      class: "Class",
      dateTime: "Date & Time",
      type: "Type",
      status: "Status",
      cancel: "Cancel",
      recurring: "Recurring",
      cannotCancel: "Cannot cancel",
      cancelPolicy: "Cancellations must be made at least 24 hours before the session.",
      noRefund: "No refund — too close to session start.",
      partialRefund: "{{percentage}}% of credits refunded ({{refunded}} credit(s)).",
      fullRefund: "Full credit refunded.",
      bookingCancelled: "Booking cancelled",
      creditRefunded: "Credit has been refunded.",
      noBookings: "No bookings yet",
    },
    profile: {
      title: "My Profile",
      personalInfo: "Personal Information",
      fullName: "Full Name",
      phone: "Phone",
      updateSuccess: "Profile updated successfully",
      updateFailed: "Failed to update profile",
      changeEmail: "Change Email",
      currentEmail: "Current Email",
      newEmail: "New Email",
      newEmailPlaceholder: "Enter new email address",
      emailChangeNote: "A confirmation link will be sent to both your current and new email address. Your email will update once you confirm.",
      emailChangeRequested: "Confirmation email sent! Check both your current and new email inbox.",
      invalidEmail: "Please enter a valid email address",
      sameEmail: "New email is the same as your current email",
    },
    promotions: {
      redeemVoucher: "Redeem Voucher",
      voucherDescription: "Enter a voucher code to receive bonus credits.",
      enterCode: "Enter voucher code…",
      redeem: "Redeem",
      voucherRedeemed: "Voucher redeemed!",
      creditsAdded: "{{credits}} credits added to your package.",
      referralTitle: "Invite Friends",
      referralDescription: "Share your referral link. When your friend signs up and gets a package, you'll earn bonus credits!",
      copy: "Copy",
      linkCopied: "Referral link copied!",
    },
  },
  notifications: {
    title: "Notifications",
    empty: "No notifications",
    markAllRead: "Mark all read",
    types: {
      booking_reminder: "Booking Reminder",
      low_credits: "Low Credits",
      package_expiry: "Expiring Soon",
      reminder: "Reminder",
      promotion: "Promotion",
      returning_customer: "Welcome Back",
    },
  },
  cancellationInfo: {
    title: "Cancellation Policy",
    atLeast: "≥ {{hours}}h before",
    lessThan: "< {{hours}}h before",
    refund: "refund",
  },
  language: {
    en: "EN",
    vi: "VI",
  },
} as const;
